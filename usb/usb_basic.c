...
void setAddress(volatile uint8_t usbAddress) {
    ...
    push_event(CALL_SET_ADDR, NULL, 0);
}
...
void controlTrans(void) {
    ...
    switch (bmbRequest) {
        case 0x8000: // Get Status (Device)
            push_event(GET_STATUS_DEVICE, NULL, 0);

            break;

        case 0x8100: // Get Status (Interface)
            push_event(GET_STATUS_INTERFACE, NULL, 0);

            break;

        case 0x8200: // Get Status (Endpoint)
            push_event(GET_STATUS_ENDPOINT, NULL, 0);

            break;

        case 0x8006:
        { // Get Descriptor(s)
            push_event(GET_DESCRIPTOR, NULL, 0);

            switch (ep0data[3]) {
                case 0x00:
                { // Undefined
                    USBE0CSR0bits.STALL = 1;
                    push_event(UNDEFINED, NULL, 0);
                }
                    break;
                ...
                { // Device Descriptor
                    ep0usbData = dev_desc;
                    push_event(SEND_DEVICE_DESCRIPTOR, dev_desc, sizeof (dev_desc));
                ...
                { // Configuration Descriptor
                    ...
                    push_event(SEND_CONFIG_DESCRIPTOR, config_desc1, sizeof (config_desc1));
                ...
                { // String Descriptors
                    switch (ep0data[2]) {
                        ...
                        { // Language ID
                            ep0usbData = string0;
                            push_event(SEND_LANGUAGE_ID, string0, sizeof (string0));
                        ...
                        { // Manufacturer
                            ep0usbData = string1;
                            push_event(SEND_MANUFACTURER, string1, sizeof (string1));
                        ...
                        { // Product
                            ep0usbData = string2;
                            push_event(SEND_PRODUCT, string2, sizeof (string2));
                        ...
                        { // Serial
                            ep0usbData = string3;
                            push_event(SEND_SERIAL, string3, sizeof (string3));
                        ...
                        { // Configuration
                            ep0usbData = string4;
                            push_event(SEND_CONFIGURATION, string4, sizeof (string4));
                        ...
                        { // Interface
                            ep0usbData = string5;
                            push_event(SEND_INTERFACE, string5, sizeof (string5));
...
void inPacket() {
    switch (bmbRequest) {
        ...
        case 0x0000: // Get Status
            push_event(GET_STATUS, NULL, 0);
            break;

        case 0x0001: // Clear Feature
            push_event(CLEAR_FEATURE, NULL, 0);
            break;

        case 0x0003: // Set Feature
            push_event(SET_FEATURE, NULL, 0);
            break;
        ...
        case 0x0005: // Save Address
            ...
            push_event(SET_ADDR, &ep0data[2], 1);
            setDevAddr = TRUE;
            break;
        ...
        case 0x0009: // Set Configuration
            push_event(SET_CONFIGURATION, NULL, 0);
            break;
