...
static void init_usb(void) {
    ...
    USBFIFOAbits.RXFIFOAD = 0x0040; // Transmit Endpoint FIFO Address bits

    USBCSR3bits.ENDPOINT = 3;
    USBFIFOAbits.TXFIFOAD = 0x0080; // Receive Endpoint FIFO Address bits

    // See data sheet DS60001320E-page 232
    USBE1CSR3bits.RXFIFOSZ = 0x4; // Transmit FIFO Size bits - 16 bytes
    USBE2CSR3bits.RXFIFOSZ = 0x6; // Transmit FIFO Size bits - 64 bytes
    USBE3CSR3bits.TXFIFOSZ = 0x6; // Receive FIFO Size bits - 64 bytes
    // 1101 = 8192 bytes 
    // 1100 = 4096 bytes
    // . 
    // . 
    ...
    USBCSR2bits.EP1RXIE = 1; // Endpoint 1 TX interrupt enable
    USBCSR2bits.EP2RXIE = 1; // Endpoint 2 TX interrupt enable
    USBCSR1bits.EP3TXIE = 1; // Endpoint 3 RX interrupt enable
