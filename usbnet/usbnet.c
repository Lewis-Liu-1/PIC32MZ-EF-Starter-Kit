...
int usbnet_get_endpoints(struct usbnet *dev, struct usb_interface *intf)
{
	...
	printk(MOD"usbnet_get_endpoints");

printk(MOD" intf->num_altsetting = %d", intf->num_altsetting);
	for (tmp = 0; tmp < intf->num_altsetting; tmp++) {
		...
		printk(MOD"alt->desc.bNumEndpoints %d", alt->desc.bNumEndpoints);
	...
	printk(MOD" in address 0x%02X",in->desc.bEndpointAddress);

	dev->in = usb_rcvbulkpipe (dev->udev,
			in->desc.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK);

	printk(MOD" out 0x%02X",out->desc.bEndpointAddress);

	dev->out = usb_sndbulkpipe (dev->udev,
			out->desc.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK);
	printk(MOD "HERE STATUS =%d", status);
	dev->status = status;
	printk(MOD "end usbnet_get_endpoints");
	return 0;
}
...
usbnet_probe (struct usb_interface *udev, const struct usb_device_id *prod)
{
	...
	printk(MOD "status = %d", status);

	if (status >= 0 && dev->status)
		status = init_status (dev, udev);

	printk(MOD "new status = %d", status);
	...
	printk(MOD "urb size %d, maxpacket %d", dev->rx_urb_size, dev->maxpacket);
	...
	printk(MOD "register status %d", status);
	if (status)
		goto out5;
